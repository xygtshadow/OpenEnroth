...
GraphicsImage *AssetsManager::getImage_BlackKey(std::string_view name) {
    std::string filename = ascii::toLower(name);

    auto i = images.find(filename);
    if (i == images.end()) {
        auto image = GraphicsImage::Create(std::make_unique<BlackKey_LOD_Loader>(pIcons_LOD, filename));
        images[filename] = image;
        return image;
    }
